...
export default {
  ...
  methods: {
    ...
    draw_value(ctx, dir, xm, y) {
      ...
      let candles = layout.candles,
          candle1 = layout.c_magnet(this.p1[0]),
          candle2 = layout.c_magnet(this.p2[0]),
          idx1 = candles.indexOf(candle1),
          idx2 = candles.indexOf(candle2);
      // console.log("range-of", idx1, idx2);
      // console.log("candle2", candle1.raw);
      let subRange = candles.slice(idx1, idx2 + 1),
          amount = subRange.reduce((ret, candle) => ret + candle.raw[7], 0),
      ...
      let b = f2(this.p2[0]) - f2(this.p1[0]) - -1;
      // Format time delta
      // Format time delta
    ...
    t2str(t) {
      ...
      let abs = Math.abs(t);
      return Math.floor(abs / 86400000);
