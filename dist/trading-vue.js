/*!
 * TradingVue.JS - v1.0.2 - Thu Dec 30 2021
...
var Grid = /*#__PURE__*/function () {
  ...
  createClass_createClass(Grid, [{
    ...
    value: function emit_cursor_coord(event) {
...
var CandleExt = /*#__PURE__*/function () {
  ...
  createClass_createClass(CandleExt, [{
    ...
    value: function draw(data) {
      ...
      this.ctx.strokeStyle = wick_color; // this.ctx.beginPath();
      // this.ctx.moveTo(x05, Math.floor(data.h));
      // this.ctx.lineTo(x05, Math.floor(data.l));
      // this.ctx.stroke();

      this.ctx.beginPath(); // draw Wick
      // this.ctx.moveTo(data.x, data.l);
      // this.ctx.lineTo(data.x, data.h);

      this.ctx.moveTo(data.x, Math.min(data.o, data.c));
      this.ctx.lineTo(data.x, data.h);
      this.ctx.moveTo(data.x, Math.max(data.o, data.c));
      this.ctx.lineTo(data.x, data.l); // draw candle

      if (!green) {
        this.ctx.fillRect(data.x - 3, Math.min(data.o, data.c), 6, Math.abs(data.o - data.c));
      } else {
        this.ctx.rect(data.x - 3, Math.min(data.c, data.o), 6, Math.abs(data.c - data.o));
      }

      this.ctx.stroke(); // if (data.w > 1.5) {
...
/* harmony default export */ const RangeToolvue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    time: function time() {
      return "time" in this.sett ? this.sett.time : true;
    },
  ...
  methods: {
    ...
    draw: function draw(ctx) {
      ...
      if (this.time) this.horizontal(ctx, x1, y1, x2, y2, xm); // console.log(`tv`, this);
    ...
    draw_value: function draw_value(ctx, dir, xm, y) {
      ...
      var candles = layout.candles,
          candle1 = layout.c_magnet(this.p1[0]),
          candle2 = layout.c_magnet(this.p2[0]),
          idx1 = candles.indexOf(candle1),
          idx2 = candles.indexOf(candle2); // console.log("range-of", idx1, idx2);
      // console.log("candle2", candle1.raw);

      var subRange = candles.slice(idx1, idx2 + 1),
          amount = subRange.reduce(function (ret, candle) {
        return ret + candle.raw[7];
      }, 0),
          turn = subRange.reduce(function (ret, candle) {
        return ret + candle.raw[8];
      }, 0),
      ...
      var amountShow = Math.round(amount / 1000000) / 100 + "亿",
      ...
      var b = f2(this.p2[0]) - f2(this.p1[0]) - -1; // Format time delta
      // Format time delta

      var dtstr = this.t2str(dt) - -1;
      var text = [];
      if (this.price) text.push("".concat(stamp1, " _ ").concat(stamp2, " ").concat(incShow, "(").concat(incpShow, ")"));
      if (this.time) text.push("".concat(b, "/").concat(dtstr, "\u5929; \u6362").concat(turnShow, "; \u91D1").concat(amountShow));
      text = text.join("\n"); // "Multiple" fillText
    ...
    t2str: function t2str(t) {
      ...
      var abs = Math.abs(t);
      return Math.floor(abs / 86400000); // let tfs = [
      //   [1000, "s", 60],
      //   [60000, "m", 60],
      //   [3600000, "h", 24],
      //   [86400000, "D", 7],
...
function stamp2str(stamp) {
  var d = new Date(stamp); // console.log("stamp is", stamp, d);

  return [p2(d.getMonth() + 1), p2(d.getDate())].join("-");
}
...
var Sidebar = /*#__PURE__*/function () {
  ...
  createClass_createClass(Sidebar, [{
    ...
    value: function calc_range() {
      ...
      var diff1 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
      var diff2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
              ...
              _vm.show_values
                ? _c("span", [
                    ...
                    _c("span", { staticClass: "t-vue-lspan" }, [
                      _vm._v(
                        "高/低 " +
                          ...
                          _vm._s(_vm.ohlcv[2]) +
                          ", " +
                          _vm._s(_vm.incOf(_vm.ohlcv[1], _vm.ohlcv[2])) +
                          "% ;"
                      )
                    ]),
                    _vm._v(" "),
                    _c("span", { staticClass: "t-vue-lspan" }, [
                      _vm._v(
                        "金额 " +
                          _vm._s(_vm.prettyNum(_vm.at[0] / 10000000)) +
                          "亿 ;"
                      )
                    ]),
                    _vm._v(" "),
                    _c("span", { staticClass: "t-vue-lspan" }, [
                      _vm._v("换 " + _vm._s(_vm.prettyNum(_vm.at[1])) + "% ;")
                    ])
                  ])
                : _vm._e(),
              _vm._v(" "),
...
/* harmony default export */ const Legendvue_type_script_lang_js_ = ({
  ...
  computed: {
    ohlcv: function ohlcv() {
      ...
      return [this.$props.values.ohlcv[1].toFixed(2), this.$props.values.ohlcv[2].toFixed(2), this.$props.values.ohlcv[3].toFixed(2), this.$props.values.ohlcv[4].toFixed(2), this.$props.values.ohlcv[5] ? this.$props.values.ohlcv[5].toFixed(0) : "n/a"];
    },
    at: function at() {
      if (!this.$props.values || !this.$props.values.ohlcv) return Array(6).fill("n/a");
      return this.$props.values.ohlcv.slice(7);
    },
  ...
  methods: {
    ...
    incOf: function incOf(value, base) {
      return Math.round((value - base) * 10000 / base) / 100;
    },
    prettyNum: function prettyNum(num) {
...
/* harmony default export */ const TradingVuevue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    showTheTip: function showTheTip(text) {
...
var WebWork = /*#__PURE__*/function () {
  ...
  createClass_createClass(WebWork, [{
    ...
    key: "relay",
    value: function () {
      var _relay = _asyncToGenerator( /*#__PURE__*/regenerator_default().mark(function _callee2(event) {
        var _this4 = this;
        ...
        return regenerator_default().wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                just = _args2.length > 1 && _args2[1] !== undefined ? _args2[1] : false;
...
var DCCore = /*#__PURE__*/function (_DCEvents) {
  ...
  createClass_createClass(DCCore, [{
    ...
    key: "range_changed",
    value: function () {
      var _range_changed = _asyncToGenerator( /*#__PURE__*/regenerator_default().mark(function _callee(range, tf) {
        var _this2 = this;
    ...
    key: "fast_merge",
    value: function fast_merge(data, point) {
...
var AggTool = /*#__PURE__*/function () {
  function AggTool(dc) {
    var _int = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 100;
